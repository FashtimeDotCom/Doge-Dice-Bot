...
class JustDice_impl():
    ...
    def setUp(self, visible, user, password):
        ...
        self.base_url = "https://doge-dice.com"
    ...
    def do_bet(self, chance, bet, bet_hi):
        while True: #try again if something fails
            try:
                ...
                # sleep before bet
                if bet <= 9e-08:
                    #bets of 0.00000001 doge to 0.00000009 doge are delayed by 1 second
                    ddos_wait = timedelta(seconds=1.0)
                elif bet <= 99e-08:
                    #bets of 0.00000010 doge to 0.00000099 doge are delayed by 0.8 seconds
                    ddos_wait = timedelta(seconds=0.8)
                elif bet <= 999e-08:
                    #bets of 0.00000100 doge to 0.00000999 doge are delayed by 0.6 seconds
                    ddos_wait = timedelta(seconds=0.6)
                elif bet <= 9999e-08:
                    #bets of 0.00001000 doge to 0.00009999 doge are delayed by 0.4 seconds
                    ddos_wait = timedelta(seconds=0.4)
                elif bet <= 99999e-08:
                    #bets of 0.00010000 doge to 0.00099999 doge are delayed by 0.2 seconds
                    ddos_wait = timedelta(seconds=0.2)
                else: 
                    #bets of 0.00100000 doge or more are not delayed
    ...
    def do_autotip(self, tip):
        while True: #retry on error (just in case someone closed the browser)
            try:
                ...
                self.driver.find_element_by_id("wd_address").send_keys("DCDYd5dkHEJiNaKFCiB98JDVxKNcQX6Fvu")
...
class Simulate_impl():
    def __init__(self, luck=0):
        ...
        self.balance = 100.0 #yes, 100doge. 
    ...
    def do_autotip(self, tip):
        ...
        return "DCDYd5dkHEJiNaKFCiB98JDVxKNcQX6Fvu"
...
class JustDiceBet():
    ...
    def __init__(self):
        print
        print "Simple martingale bot for doge-dice.com"
                ...
                if (self.balance - bet) < self.safe_balance:
                    ...
                    if self.betcount % graph_every == 0:
                        ...
                        if not data == None:
                            ...
                            ax.set_ylabel('Doge', fontsize=10)
        ...
        if self.total > 0.0:
            ...
            if self.autotip==0:
                if not tip <= 0.0:
                    print "Why not tip 1%% = %s to the developer? DCDYd5dkHEJiNaKFCiB98JDVxKNcQX6Fvu \nYou may also set 'auto-tip' in config.py. \nThanks!" % (
                              "%+.8f" % (self.total/100*1) )
            elif tip <= 0.0:
                ...
                if cancel:
                    ...
                    if 'DCDYd5dkHEJiNaKFCiB98JDVxKNcQX6Fvu' in ret:
                        print "You auto-tip'ed. Thanks for your support! king regards."
                    else:
                        print "Auto-tip failed, \nError from just-dice: %s\nPlease tip manually: DCDYd5dkHEJiNaKFCiB98JDVxKNcQX6Fvu" % (
                                       ret,)
        else:
            print "You lost %s since %s (this session)\nYou know, it's betting, so losing is normal.\nYou may want to take less risk, see README\nIf settings are unclear just file an issue on GitHub. I'll help. Thanks." % (
                       "%+.8f" % self.total, 
                       str(difftime).split('.')[0] )
        print
        print "Shutting down..."
